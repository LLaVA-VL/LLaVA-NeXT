...
ABLATION_DATA="inhouse_edu_math"
...
torchrun --nproc_per_node="${ARNOLD_WORKER_GPU}" --nnodes="${ARNOLD_WORKER_NUM}" --node_rank="${ARNOLD_ID}" --master_addr="${METIS_WORKER_0_HOST}" --master_port="${port_in_cmd}" \
    ...
    --model_name_or_path $LLM_VERSION \
    --version $PROMPT_VERSION \
    --data_path="/mnt/bn/${NAS_REGION}/data/llava_instruct/{llava_158k_detailv3_reinstall_gpt4v24k_wild15k_mixdocvqa_dca45k_synden40k_cococaps20k_sg40kt2k_ori,edu_math_ch/instruct_edu_math_ch_100K}.json" \
