...
FINAL_RUN_NAME="dist1_llavanext-${LLM_VERSION_CLEAN}-mlp2x_gelu-pretrain_blip558k_plain-finetune_blip558k_fvis-finetune_la2mix_10pct_fvis_direct32k_${ratio}"
...
if [ ! -f "/mnt/bn/${NAS_REGION}/checkpoints/${FINAL_RUN_NAME}/config.json" ];then
    torchrun --nproc_per_node="${ARNOLD_WORKER_GPU}" --nnodes="${ARNOLD_WORKER_NUM}" --node_rank="${ARNOLD_ID}" --master_addr="${METIS_WORKER_0_HOST}" --master_port="${port_in_cmd}" \
        ...
        --data_path "/mnt/bn/${NAS_REGION}/workspace/boli01/projects/LLaVA_Next/scripts/llava2_mix_full.yaml" \
...
accelerate launch --num_processes 8 --main_process_port 12345 -m lmms_eval \
    ...
    --tasks mme,mmbench_en_dev,mmmu_val,infovqa_val,ai2d,chartqa,docvqa_val,scienceqa_img,vizwiz_vqa_val,pope,ok_vqa \
    --batch_size 1 \
    --log_samples \
    --log_samples_suffix 10pct_llava2mix \
    --output_path ./logs/
