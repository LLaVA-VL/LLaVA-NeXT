...
for episode in tqdm(data):
    ...
    for i in range(trajectory_length):
        ...
        """
        # print(question)
        # print(len(input_images[max(0, i - history_context_length + 1) : i + 1]))
...
with open(f"outputs/{MODEL_NAME}_val_seen_results.json", 'w') as f:
    json.dump(paths, f)

with open(f"outputs/{MODEL_NAME}_val_seen_gt.json", 'w') as f:
