...
pretrained = "https://huggingface.co/lmms-lab/llama3-llava-next-8b"
model_name = "llava_llama3"
tokenizer, model, image_processor, max_length = load_pretrained_model(pretrained, None, model_name, device_map=self.device_map, **llava_model_args) # Add any other thing you want to pass in llava_model_args
...
image_tensor = process_images(flattened_visuals, image_processor, model.config).to(dtype=torch.float16, device=self.device)

conv_template = "llava_llama3" # Make sure you use correct chat template for different models
...
input_ids = tokenizer_image_token(prompt_question, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").to(self.device)
